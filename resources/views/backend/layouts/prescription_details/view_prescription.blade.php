    ...
    <div class="container">
        ...
        <div class="row">
            <div class="col-md-4 offset-md-1 diagnosis">
                ...
                <ul>
                    <li>{{$prescription->chief_complaint}}</li>
                </ul>
                <h5>History</h5>
                <ul>
                <li>Weight: {{$prescription->weight}}</li>
                <li>Blood pressur: {{$prescription->bp}}</li>
                </ul>
                <h5>Investigation</h5>
                <ul>
                @foreach($prescription->prescribe_medicinerelation as $med)
       
                    <li>{{optional($med->diagnosisrelation)->name}}</li>
                    @endforeach
                </ul>
            </div>
            ...
            <h5>Medicine</h5>
                ...
                @foreach($prescription->prescribe_medicinerelation as $med)
                    ...
                    <li>{{optional($med->medicinerelation)->name}} <br> {{$med->dosages}} ------------ {{$med->days}}</li>
                @endforeach
                ...
                <h5>Instruction</h5>
                <ul>
                    <li>{{$prescription->additional_instructions}}</li>
